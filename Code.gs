...
/* 시트 컬럼(12개):
 *   [ID(0), 주제(1), 표현(2), 뜻(3),
 *    여행예문(4), 친구예문(5), 학교예문(6), 가족예문(7),
 *    동화1(8), 동화2(9), 동화3(10), 이미지(11)]
 * 실제 예문은 [여행, 친구, 학교, 가족] 4 슬롯이며 위치 자체가 장면을 의미.
 * 비어 있는 슬롯은 결과에서 제외된다. */
const REAL_SCENES_ = ['여행', '친구', '학교', '가족'];
...
function readEntries_(sheetName) {
  ...
  return values
    ...
    .map(row => {
      // 장면별 예문 (빈 문자열도 키는 유지)
      const realByScene = {
        '여행': String(row[4] || '').trim(),
        '친구': String(row[5] || '').trim(),
        '학교': String(row[6] || '').trim(),
        '가족': String(row[7] || '').trim()
      };
      // 비어 있지 않은 예문만 평탄화
      const realExamples = REAL_SCENES_
        .map(s => realByScene[s])
        .filter(Boolean);
      const folkExamples = [row[8], row[9], row[10]]
        .map(v => String(v || '').trim())
        .filter(Boolean);
      return {
        id:            row[0],
        ...
        example:       realExamples[0] || '',
        realExamples:  realExamples,
        realByScene:   realByScene,
