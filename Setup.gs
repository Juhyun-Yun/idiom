...
const ENTRY_COLS = 12; // ID, 주제, 표현, 뜻, 여행/친구/학교/가족 예문, 동화1~3, 이미지

// 실제 예문은 [여행, 친구, 학교, 가족] 순서로 위치 고정. 위치 자체가 장면을 의미한다.
const SCENE_LABELS = ['여행', '친구', '학교', '가족'];

/**
 * 시트 컬럼 순서 (12개):
 *   [ID, 주제, 표현, 뜻, 여행예문, 친구예문, 학교예문, 가족예문, 동화1, 동화2, 동화3, 이미지]
 *
 * 시드와 시트 순서가 동일하므로 그대로 잘라 반환.
 * 옛 11컬럼 시드(가족 예문 없음)도 호환되게 처리한다.
 */
function reorderSeedRow_(row) {
  if (row.length >= ENTRY_COLS) return row.slice(0, ENTRY_COLS);
  // 옛 11컬럼 시드: 가족 예문 자리(인덱스 7)에 빈 문자열 끼워 넣기
  return [
...
function ensureIdiomsSheet_(ss) {
  ...
  if (sheet.getLastRow() === 0) {
    ...
    sheet.setColumnWidths(5, 7, 280); // 여행/친구/학교/가족 + 동화 예문 3개 (5~11열)
    sheet.setColumnWidth(12, 200); // 이미지
  }
